...
def _get_length_reference(ref_lengths, cand_length, strategy="best_match"):
    ...
    "选择长度相差最小的，如果长度差值一样，就找ref中长度最短的"
...
# def zq_bleu_digit(cand,refs,bp = "closest",smooth=False,n=4,weights=None):
#     '''仿照下面的bleu写的，但是cand中是list of list,ecah element in the inner list shall be a digit
#     cand = [ [88,29299,731],
#              [789798,35300.67] ]
#     there is an example above for cand
#     '''
#     len_c = 0
#     len_ref = 0
...
def bleu(cand, refs, bp='closest', smooth=False, n=4, weights=None):
    ...
    for candidate, references in zip(cand, refs):
        len_c += len(candidate)
        len_ref += _get_length_reference([len(r) for r in references], len(candidate),
                                         strategy='best_match' if bp == 'closest' else 'min') #获得于candidate长度相差最小的ref的长度中最短的
        ...
        for reference in references:
            ...
            for ngram in cngrams:
                ...
                if ngram in rngrams:
                    ...
                    # temp = min(rngrams[ngram], cngrams[ngram])
                    # if temp > 0:
                    #     print(temp,"= temp")
                    #     exit(67)
                    #查看了程序之后发现，不是逻辑错误，是的的确确一个common的片段都没有。min(rngrams[ngram], cngrams[ngram]) = 0
        for ngram in cngrams:
            nl = len(ngram.split())
            tngram_corpus[nl] += tngram_sample[ngram]
    ...
    # BLEU: candidate prefered to be shorter, so penalize long translations
    # N-gram的匹配度可能会随着句子长度的变短而变好，因此会存在这样一个问题：一个翻译引擎只翻译出了句子中部分句子且翻译的比较准确，那么它的匹配度依然会很高。为了避免这种评分的偏向性，BLEU在最后的评分结果中引入了长度惩罚因子(Brevity Penalty)。
    if len_c <= len_ref:
        lp = math.exp(1. - len_ref * 1. / len_c)
    ...
    for i in range(n):
        print("i = ",i, "multi_bleu = ",multi_bleu[i+1])
        _safe_log(multi_bleu[i+1])
