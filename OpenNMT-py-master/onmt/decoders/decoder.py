...
class RNNDecoderBase(DecoderBase):
    ...
    def init_state(self, src, memory_bank, encoder_final):
        ...
        self.state["input_feed"] = \
            ...
            #在一般的配置的情况下，主要是为了初始化self.state['input_feed']这个张量为全0.
            #这个张量会跟tgt(decoder端的输入)在hidden_size维度上拼接起来。
            #还为了初始化self.state["hidden"], 这个会作为rnn的第一个隐层状态
...
class InputFeedRNNDecoder(RNNDecoderBase):
    ...
    def _run_forward_pass(self, tgt, memory_bank, memory_lengths=None):
        ...
        # print("input_feed = ",input_feed.size()) #[batch_size, rnn_hidden_size]
        input_feed_batch, _ = input_feed.size() #batch_size, rnn_hidden_size
        _, tgt_batch, _ = tgt.size() #tgt_len, batch_size, hidden_size
        aeq(tgt_batch, input_feed_batch) #证明两者相等
        # END Additional args check.

        dec_outs = []
        attns = {}
        if self.attn is not None:
            attns["std"] = [] #从最终结果来看是有std这个键的
        ...
        for emb_t in emb.split(1):#tensor.split(1)在第0个维度上将原来的tensor切开，返回一个tuple，有tgt_len个元素。每个元素.size() =[1,batch_size, emb_dim]
            # import pdb
            # pdb .set_trace()
            decoder_input = torch.cat([emb_t.squeeze(0), input_feed], 1) #input_feed.size() = [3,100]=[batch_size,rnn_size]
            rnn_output, dec_state = self.rnn(decoder_input, dec_state) #dec_state.size(1,3,100) =[num_layers, batch_size , hidden_size * num_direaction]
