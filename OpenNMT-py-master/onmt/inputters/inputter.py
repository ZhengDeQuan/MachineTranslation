...
def build_dataset_iter(corpus_type, fields, opt, is_train=True):
    ...
    batch_fn = max_tok_len if is_train and opt.batch_type == "tokens" else None #max_tok_len居然是一个函数
    batch_size_multiple = 8 if opt.model_dtype == "fp16" else 1

    device = "cuda" if opt.gpu_ranks and torch.cuda.is_available() else "cpu"
