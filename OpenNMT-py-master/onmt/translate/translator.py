...
class Translator(object):
    ...
    def translate(
        ...
        for batch in data_iter:
            # type(batch)=<class 'torchtext.data.batch.Batch'>
            # print("batch = ") # 我想看看batch是什么
            # print(batch)
            #batch.src[0].size() = [59,30,1] =[src_len, batch_size,1]
            #batch.src[1].size() = 30 各个句子的长度
            #batch.indices.size() = 30,前30个被shuffle的indices

            # import pdb
            # pdb.set_trace()
            batch_data = self.translate_batch(
                batch, data.src_vocabs, attn_debug
            )#这个batch_data应该是被翻译过的
            #type(batch_data)=dict, batch_data.keys() = dict_keys(['predictions', 'scores', 'attention', 'batch', 'gold_score'])
            #batch_data["predictions"]:type=list,len=30,ele0:type=list,len=1,ele0:type=tensor,size=src_len
            #batch_data["scores"]:type=list,len=30,ele0:type=list,len=1,ele0:type=tensor,size=1
            translations = xlation_builder.from_batch(batch_data) #len(translations)=30,type = list ,ele = <class 'onmt.translate.translation.Translation'>

            for trans in translations:
                all_scores += [trans.pred_scores[:self.n_best]]
                pred_score_total += trans.pred_scores[0]
                pred_words_total += len(trans.pred_sents[0]) #trans.pred_sents = [['Black', 'box', 'in', 'your', 'car']]
                if tgt is not None: #tgt提供了之后，才能计算得到gold_score
        ...
        if self.report_score: #default True
            msg = self._report_score('PRED', pred_score_total,
                                     pred_words_total)
            self._log(msg)
            if tgt_path is not None: #tgt default is None
                msg = self._report_score('GOLD', gold_score_total,
                                         gold_words_total)

                self._log(msg)
                if self.report_bleu:
                    print("yes report bleu")
    ...
    def translate_batch(self, batch, src_vocabs, attn_debug):
        ...
        with torch.no_grad():
            if self.beam_size == 1: #self.beam_size default in opts.py =5
    ...
    def _decode_and_generate(
        ...
        #dec_out.size()=[1,150,500]
        #dec_attn['std'].size()=[1,150,59]
        # Generator forward.
    ...
    def _report_bleu(self, tgt_path):
        ...
        print()
        # print("in _report_bleu")
        # tgt_path = "../HindiEng/dev_test/test.en.zq"
        # import pdb
        # pdb.set_trace()
    ...
    def _report_rouge(self, tgt_path):
        ...
        #print("in _report_rouge pwd = ",os.getcwd())#/home/nemo/zhengquan/OpenNMT-py
